# Получает с сайта последнюю версию Visual Studio Code и если у нас нет такой версии, то сначала копирует шаблон устанвоки PSADT, заменяет в неём значения. После этого создаёт приложение в SCCM с заполнением всех нужых атрибутов.

#region Function
function Get-VisualStudioCode ($URLPage){
    ...
    # try to find update
    $a = $res.Content.IndexOf("Update $global:Version")
    if ($a -ne '-1')
    {
    ...
    $global:FileName_x64 = "VSCodeSetup-x64-$version.exe"
    $global:FileName_x86 = "VSCodeSetup-x86-$version.exe"
...
$global:Application = "Visual Studio Code"
$global:Application_detection = 'Visual Studio Code'
$global:DirAppinConsole = "Visual Studio Code"
$global:DistribPath = "$($Config.DistribPath)\Visual Studio Code"
$global:FileName_x64 = 'VSCodeSetup-x64-%version%.exe'
$global:FileName_x86 = 'VSCodeSetup-x86-%version%.exe'
$URLPage = 'https://code.visualstudio.com/updates' # For Windows only
$global:Version = '1.0'
$ProxyURL = $Config.ProxyURL
$global:PSADTTemplatePath = $Config.PSADTTemplatePath
$global:body = ''
$global:NewVersion = $null
$global:templatename = 'Config_VSCode_template.ps1'
$To= $Config.To
#endregion Variables
try{
    Get-VisualStudioCode -URLPage $URLPage
    if ($global:NewVersion -eq $true)
    {
        ...
        $NewApplication_Properties = @{
            ...
            LocalizedDescription = 'Visual Studio Code - Редактор кода. Включает в себя отладчик, инструменты для работы с Git, подсветку синтаксиса, IntelliSense и средства для рефакторинга. Имеет широкие возможности для кастомизации: пользовательские темы, сочетания клавиш и файлы конфигурации.'
            IconLocationFile = "\\SCCMServer\Sources\Applications\Visual Studio Code\icon.png"
