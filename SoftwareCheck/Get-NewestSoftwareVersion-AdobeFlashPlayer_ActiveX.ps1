# Получает с сайта последнюю версию Adobe Flash Player ActiveX и если у нас нет такой версии, то сначала копирует шаблон устанвоки PSADT, заменяет в неём значения. После этого создаёт приложение в SCCM с заполнением всех нужых атрибутов.

#region Function
# Получение последней версии Adobe Flash Player ActiveX
function Get-AdobeFlashPlayerActiveX ($URLPage){
    ...
    Write-Host "Download path: $DistribPath\$script:Version\install_flash_player_${majorVersion}_active_x.msi"#`n$DistribPath\$script:Version\install_flash_player_${majorVersion}_plugin.msi`n$DistribPath\$script:Version\install_flash_player_${majorVersion}_ppapi.msi`n"
    $script:FileName = "install_flash_player_${majorVersion}_active_x.msi"
...
$script:Application = "Adobe Flash Player ActiveX"
$script:Application_detection = "Adobe Flash Player*ActiveX"
$script:DirAppinConsole = "Adobe Flash Player"
$script:DistribPath = "$($Config.DistribPath)Adobe\Flash Player ActiveX"
...
$script:templatename = 'Config_AdobeFlashPlayerActiveX_template.ps1'
$To= $Config.To
#endregion Variables
