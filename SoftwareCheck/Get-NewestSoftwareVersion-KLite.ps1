# Получает с сайта последнюю версию K-Lite Codec Pack Standard и если у нас нет такой версии, то сначала копирует шаблон устанвоки PSADT, заменяет в неём значения. После этого создаёт приложение в SCCM с заполнением всех нужых атрибутов.

#region Function
function Get-KLite ($URLPage){
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $result = Invoke-WebRequest $URLPage -Proxy $ProxyURL -ProxyUseDefaultCredentials -UseBasicParsing
    ($result.RawContent -match 'Version [0-9\.]+ Standard')
    $global:Version = $Matches[0] -replace 'Version ', '' -replace ' Standard', ''
    $DownloadURL = ($result.Links | Where-Object {$_.OuterHTML -like '*Mirror 1*'}).href
...
$global:Application = "K-Lite Codec Pack Standard"
$global:Application_detection = "K-Lite Codec Pack*Standard"
$global:DirAppinConsole = "K-Lite Codec Pack"
$global:DistribPath = "$($Config.DistribPath)\K-Lite Codec"
$global:FileName = 'K-Lite_Codec_Pack_Standard.exe'
