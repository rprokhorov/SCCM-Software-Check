# Получает с сайта последнюю версию PuTTY и если у нас нет такой версии, то сначала копирует шаблон устанвоки PSADT, заменяет в неём значения. После этого создаёт приложение в SCCM с заполнением всех нужых атрибутов.

#region Function
function Get-PuTTY ($URLPage){
    ...
    $global:FileName_x64 = "PuTTY-x64-$version.msi"
    $global:FileName_x86 = "PuTTY-x86-$version.msi"
...
$global:Application = "PuTTY"
$global:Application_detection = 'PuTTY'
$global:DirAppinConsole = "PuTTY"
$global:DistribPath = "$($Config.DistribPath)\PuTTY"
$global:FileName_x64 = "PuTTY-x64-$version.msi"
$global:FileName_x86 = "PuTTY-x86-$version.msi"
...
$global:templatename = 'Config_Putty_template.ps1'
$To= $Config.To
#endregion Variables

try{
    Get-PuTTY -URLPage $URLPage
    if ($global:NewVersion -eq $true)
    {
        ...
        $NewApplication_Properties = @{
            ...
            LocalizedDescription = 'PuTTY — свободно распространяемый клиент для различных протоколов удалённого доступа, включая SSH, Telnet, rlogin'
