# Получает с сайта последнюю версию mRemoteNG и если у нас нет такой версии, то сначала копирует шаблон устанвоки PSADT, заменяет в неём значения. После этого создаёт приложение в SCCM с заполнением всех нужых атрибутов.

#region Function
function Get-mRemoteNG ($URLPage){
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $result = Invoke-WebRequest $URLPage -UseBasicParsing -Proxy $ProxyURL -ProxyUseDefaultCredentials
    ($result.Links | ? {$_.href -like '*msi*'})[0].href -match '[0-9\.]+msi' | Out-Null
    $global:Version = $Matches[0] -replace '.msi'
    $global:FileName = $global:FileName -replace '%version%', $global:version
    $DownloadURL = ($result.Links | ? {$_.href -like '*msi*'})[0].href
...
$global:Application = "mRemoteNG"
$global:Application_detection = 'mRemoteNG'
$global:DirAppinConsole = "mRemoteNG"
$global:DistribPath = "$($Config.DistribPath)\mRemoteNG"
$global:FileName = 'mRemoteNG-%Version%.msi'
...
$global:Publisher = 'Next Generation Software'
$global:templatename = 'Config_mRemoteNG_template.ps1'
$To= $Config.To
#endregion Variables

Try{
    Get-mRemoteNG -URLPage $URLPage
