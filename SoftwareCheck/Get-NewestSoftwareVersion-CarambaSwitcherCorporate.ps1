#region Function
# Получение последней версии Caramba Switcher Corporate
...
function Get-CarambaSwitcherCorporate ($URLPage){
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $result = Invoke-WebRequest $URLPage -UseBasicParsing -Proxy $ProxyURL -ProxyUseDefaultCredentials
    $DownloadURL = ($result.Links | ? {$_.href -like '*CarambaSwitcherCorporate.exe'}).href
    ($result.Links | ? {$_.href -like '*CarambaSwitcherCorporate.exe'}).outerHTML -match 'Corporate [0-9\.]+'
    $script:Version = $Matches[0] -replace 'Corporate '
...
$script:Application = "Caramba Switcher Corporate"
$script:Application_detection = 'Caramba Switcher Corporate'
$script:DirAppinConsole = "Caramba Switcher Corporate"
$script:DistribPath = "$($Config.DistribPath)Caramba Switcher Corporate"
$script:FileName = 'CarambaSwitcherCorporate-%Version%.exe'
$URLPage = 'https://caramba-switcher.com/#contacts'
$script:Version = '1.1'
...
#endregion Variables
Try{
    Get-CarambaSwitcherCorporate -URLPage $URLPage
