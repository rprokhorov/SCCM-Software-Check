# Получает с сайта последнюю версию Nmap и если у нас нет такой версии, то сначала копирует шаблон устанвоки PSADT, заменяет в неём значения. После этого создаёт приложение в SCCM с заполнением всех нужых атрибутов.

#region Function
function Get-Nmap ($URLPage){
...
$script:Application = "Nmap"
$script:Application_detection = 'Nmap'
$script:DirAppinConsole = "Nmap"
$script:DistribPath = "$($Config.DistribPath)Nmap"
$script:FileName = 'Nmap-%Version%.exe'
...
$script:Publisher = 'Nmap Project'
$script:templatename = 'Config_Nmap_template.ps1'
$To= $Config.To
#endregion Variables

Try{
    Get-Nmap -URLPage $URLPage
    if ($script:NewVersion -eq $true)
    {
        ...
        $NewApplication_Properties = @{
            ...
            LocalizedDescription = "Nmap - свободная утилита, предназначенная для разнообразного настраиваемого сканирования IP-сетей с любым количеством объектов, определения состояния объектов сканируемой сети (портов и соответствующих им служб)."
