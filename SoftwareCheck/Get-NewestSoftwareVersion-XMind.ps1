...
function Get-XMind ($URLPage){
    ...
    $DownloadURL = "https:$(($result.Links | Where-Object {$_.href -like '*xmind*-windows.exe'}).href)"
    # на сайте нет указания точной новой версии
    New-Item -ItemType Directory $DistribPath -ErrorAction SilentlyContinue
    Invoke-WebRequest -Uri $DownLoadURL -OutFile "$DistribPath\check_version.exe" -UseBasicParsing -UserAgent [Microsoft.PowerShell.Commands.PSUserAgent]::FireFox #-Proxy $ProxyURL -ProxyUseDefaultCredentials 
    $script:Version = (Get-ItemProperty -Path "$DistribPath\check_version.exe").VersionInfo.ProductVersion -replace ' '
...
$script:DistribPath = "$($Config.DistribPath)XMind"
$script:FileName = 'XMind-%Version%.exe'
...
$script:Publisher = 'XMind Ldt.'
$script:templatename = 'Config_XMind_template.ps1'
$To= $Config.To
$From = $config.From
