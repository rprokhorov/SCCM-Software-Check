# Получает с сайта последнюю версию XMind и если у нас нет такой версии, то сначала копирует шаблон устанвоки PSADT, заменяет в неём значения. После этого создаёт приложение в SCCM с заполнением всех нужых атрибутов.

#region Function
function Get-XMind ($URLPage){
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $result = Invoke-WebRequest $URLPage -UseBasicParsing -Proxy $ProxyURL -ProxyUseDefaultCredentials
    $DownloadURL = ($result.Links | Where-Object {$_.href -like '*xmind*-windows.exe'}).href -replace '//'
    # на сайте нет указания точной новой версии
    Invoke-WebRequest -Uri $DownLoadURL -OutFile "$DistribPath\check_version.exe" -UseBasicParsing -Proxy $ProxyURL -ProxyUseDefaultCredentials -UserAgent [Microsoft.PowerShell.Commands.PSUserAgent]::FireFox
    $global:Version = (Get-ItemProperty -Path "$DistribPath\check_version.exe").VersionInfo.ProductVersion -replace ' '
    ...
    if (Test-Path "filesystem::$DistribPath\$version\")
    {
        ...
        Remove-Item -Path "$DistribPath\check_version.exe" -Force
    }
    Else
    {
        ...
        Move-Item -Path "$DistribPath\check_version.exe" -Destination $destination -Force
    }
    Write-Host "-===========-" -ForegroundColor Green
}
...
$global:Application = "XMind"
$global:Application_detection = 'XMind'
$global:DirAppinConsole = "XMind"
$global:DistribPath = "$($Config.DistribPath)\XMind"
$global:FileName = 'XMind-%Version%.exe'
$URLPage = 'https://www.xmind.net/download/xmind8'
$global:Version = '1.1'
$ProxyURL = $Config.ProxyURL
$global:PSADTTemplatePath = $Config.PSADTTemplatePath
$global:body = ''
$global:NewVersion = $null
$global:Publisher = 'XMind Ldt.'
$global:templatename = 'Config_XMind_template.ps1'
$To= $Config.To
#endregion Variables

Try{
    Get-XMind -URLPage $URLPage
