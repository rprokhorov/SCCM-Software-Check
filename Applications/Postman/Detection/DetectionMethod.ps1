...
if ((Get-ItemProperty -Path "C:\Program Files\Postman\Postman.exe" -ErrorAction SilentlyContinue).VersionInfo.ProductVersion -eq $appDetectionVersion)
{
	write-log -Message "Software version $((Get-ItemProperty -Path "C:\Program Files\Postman\Postman.exe" -ErrorAction SilentlyContinue).VersionInfo.ProductVersion)" -MessageType Info -Component Main -LogFileName $LogName
    Write-Log -Message "Software was found" -MessageType Info -Component Main -LogFileName $LogName
    Write-Host "Software was found"
}
else {
	write-log -Message "Software version $((Get-ItemProperty -Path "C:\Program Files\Postman\Postman.exe" -ErrorAction SilentlyContinue).VersionInfo.ProductVersion)" -MessageType Error -Component Main -LogFileName $LogName
	Write-Log -Message "Software was not found" -MessageType Error -Component Main -LogFileName $LogName
}
